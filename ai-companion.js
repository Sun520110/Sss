...
const DEFAULT_SETTINGS = {
    ...
    'character_card_id': 'default_assistant'
};
...
class AICompanion {
    ...
    async initialize() {
        try {
            ...
            // 初始化角色卡管理器并应用当前角色卡
            if (typeof characterCardManager !== 'undefined') {
                await characterCardManager.init();
                await this.applyCharacterCard();
            }
    ...
    // 应用角色卡到当前设置
    async applyCharacterCard(cardId = null) {
        if (typeof characterCardManager === 'undefined') return;
        
        const id = cardId || this.settings.character_card_id;
        const card = characterCardManager.getCard(id);
        
        if (card) {
            // 更新当前设置
            this.settings.ai_name = card.name;
            this.settings.ai_avatar = card.avatar;
            this.settings.ai_avatar_type = card.avatar_type;
            this.settings.ai_gender = card.ai_gender;
            this.settings.personality_id = card.personality_id;
            this.settings.custom_prompt = card.custom_prompt;
            this.settings.identity = card.identity;
            this.settings.relationship = card.relationship;
            this.settings.tone = card.tone;
            this.settings.response_length = card.response_length;
            this.settings.use_emojis = card.use_emojis;
            this.settings.chat_background = card.chat_background;
            this.settings.character_card_id = card.id;
            
            // 保存设置
            await this.saveSettings();
            
            // 更新角色卡管理器的当前角色卡
            characterCardManager.switchCard(card.id);
        }
    }
    
    // 获取当前角色卡
    getCurrentCharacterCard() {
        if (typeof characterCardManager === 'undefined') return null;
        return characterCardManager.getCurrentCard();
    }
